...
efficiency losses concentrated on smooth, well-scaled targets. A native C production
...
\emph{(A) Representation: a chart class is missing.} The shell target
...
set and needs a radial chart that does not yet exist. The extreme-bend
banana ($b{=}0.2$, $d{=}30$; no method passes) exceeds what composed
pairwise quadratic shears express at this budget.

\emph{(B) Acquisition: the chart class exists, but warm-up data never
exposes the structure it must fit.} This is the dominant failure
mechanism. The horseshoe's triangular chart requires per-coordinate
scale (local-shrinkage) variation for its driver scan to fire, and the
L-BFGS-anchored pool sits near the mode where none is visible; the
$\alpha{=}6$ skew target's triangular chart \emph{is} admitted, but
forward-KL responsibilities concentrate on the co-located Gaussian
components and the fit, made from near-degenerate importance weights,
is poor (production acceptance $0.02$). Deep hierarchies
(\texttt{hgauss60}: $15\sigma$ median error), both funnels, and
eight-schools fail the same way at growing dimension, and the
Rosenbrock valley is the purest case: the shear \emph{fitter} recovers
the correct chart when handed oracle valley data, but warm-up
exploration never collects such data (the success-history DE explorer
ablation returned null, Section~\ref{sec:negative}). In every
acquisition-class failure we measured, production acceptance collapses
($\le0.06$, versus $\ge0.4$ on winning rows)---a deployment-observable
symptom requiring no reference draws.
...
The remedies map one-to-one: (A) a radial chart and richer shear
elements; (B) visited-state refits and population-based acquisition
(Section~\ref{sec:lim}); (C) is inherent---the correct reading is that
PMR-HMC's advantage is a function of how much structure the posterior
has for an atlas to remove; (D) is shared by all methods.

\begin{table}[t]
\caption{Every non-win row, diagnosed. Class: A $=$ missing chart
class, B $=$ warm-up acquisition, C $=$ no-advantage regime
(gate-passing efficiency loss), D $=$ reference-limited. ``none'' $=$
no method passes the gate on that row.}
\label{tab:failures}
\centering\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llll}
\toprule
row(s) & winner & class & diagnosed cause\\
\midrule
shell & MCLMC & A & polar chart is planar; needs radial chart\\
banana $b{=}0.2$, $d{=}30$ & none & A & bend exceeds composed-shear family at budget\\
horseshoe ($d{=}15$) & MCLMC & B & pool lacks shrinkage variation; driver scan silent\\
skew $\alpha{=}6$ & MCLMC & B & tri chart starved by Gaussian responsibilities\\
hgauss60, funnels, 8-schools & none/NUTS & B & scale-coupling unexposed at budget; acc $\le0.06$\\
Rosenbrock $d{=}10$ & none & B & valley data never acquired (fitter itself correct)\\
...
production BCF $d{=}49$ & dNUTS & C & dense mass captures campaign-block covariance\\
...
dimension ceiling near $d\approx400$ under fixed budgets;
block-structured atlas construction is the natural remedy. (4) Valley
