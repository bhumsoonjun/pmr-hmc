...
\begin{table}[t]
\caption{Representative posteriordb rows, gold-judged (min-ESS per
$10^3$ target-oracle units; PMR medians over 8 seeds; ratio $=$
PMR/diagonal-NUTS and PMR/dense-NUTS; err $=$ max standardized
mean\,/\,log-variance error versus gold draws; $^\ddagger$excluded from
the win count as a PMR accuracy failure. Full $37\times10$ panel in
Appendix~\ref{app:panels}.)}
\label{tab:pdb}
\centering\small
\begin{tabular}{lrrrrll}
\toprule
posterior & $d$ & PMR & NUTS & dNUTS & ratio & PMR err\\
\midrule
\input{pdb_top}
\end{tabular}
\end{table}
...
win. An earlier single-seed version of this table overstated the
surrogate's contribution; these 8-seed medians supersede it.

\begin{table}[t]
\caption{Decomposition ablations (min-ESS per $10^3$ units; medians
over 8 seeds; parenthesized $=$ max standardized mean\,/\,log-variance
error; $^{*}$ fails the accuracy gate---a starred rate is not a valid
win at any magnitude). The funnel row is gate-failed for every
variant.}
\label{tab:decomp}
\centering\small
\begin{tabular}{lrrrr}
\toprule
target & full & zero-force & global-only & NUTS\\
\midrule
banana20 & 46.0 (.05/.09) & \textbf{52.9} (.06/.12) & 19.5$^{*}$ (.28/1.5) & 0.5$^{*}$ (.24/.88)\\
funnel10 & 2.3$^{*}$ (.06/1.6) & 6.8$^{*}$ (.08/1.5) & 0.6$^{*}$ (.24/2.4) & 0.1$^{*}$ (.41/1.3)\\
mixture2 & 43.2 (.03/.04) & 65.9 (.03/.04) & \textbf{497.0} (.03/.03) & 61.1$^{*}$ (.65/3.4)\\
ring20 & \textbf{63.7} (.05/.07) & 45.4 (.06/.09) & 0.4$^{*}$ (.56/.64) & 2.1 (.04/.12)\\
\bottomrule
\end{tabular}
\end{table}
